...
##
## Option B patch: RGB-native input (3 channels instead of packed 4-ch RAW),
## up3 stage removed so the network's total upsampling factor matches (4x
## instead of 8x). See burstormer-gopro-pipeline-code.md for the rationale.
##
## PL2 patch (this file): the original Option B version used PL-1.5-era hook
## signatures -- validation_epoch_end(self, outs) and
## optimizer_zero_grad(..., optimizer_idx) -- which don't exist in modern
## pytorch_lightning (2.x). This version uses the current hook shapes:
##   - validation_step now appends each batch's PSNR to a list on self,
##     and on_validation_epoch_end(self) (no args) reduces + logs it, then
##     clears the list for the next epoch.
##   - optimizer_zero_grad drops the removed optimizer_idx argument.
## Only needed if you're running a modern torch/pytorch_lightning install
## (Path B in the setup doc) rather than the repo's pinned install.yml env.
...
class Burstormer(pl.LightningModule):
    def __init__(self, num_features=48, burst_size=8, reduction=8, bias=False):
        ...
        # PL2 patch: PL 1.5's validation_epoch_end(self, outs) auto-collected
        # every validation_step return value for you. PL 2.x removed that --
        # on_validation_epoch_end(self) takes no arguments, so we accumulate
        # per-batch results ourselves and clear the list each epoch.
        self.validation_step_outputs = []

        bias = False
        
        # Option B: RGB burst input instead of packed 4-ch RAW mosaic.
        ...
        # up3 removed: RGB input has no implicit 2x from Bayer packing, so the two
        # x2 stages above (up1, up2) already reach the 4x SR target instead of three.
    ...
    def forward(self, burst):
        ...
        # up3 removed -- feed directly into out_conv (see __init__)
    ...
    def validation_step(self, val_batch, batch_idx):
        ...
        # PL2 patch: stash the result instead of relying on PL to collect it.
        self.validation_step_outputs.append(PSNR)
        return PSNR

    def on_validation_epoch_end(self):
        # PL2 patch: replaces validation_epoch_end(self, outs). No outputs
        # argument is passed anymore -- read back what validation_step stashed.
        PSNR = torch.stack(self.validation_step_outputs).mean()
    ...
    def optimizer_zero_grad(self, epoch, batch_idx, optimizer):
        # PL2 patch: optimizer_idx was removed from this hook's signature
        # (multi-optimizer indexing this way no longer exists in PL2 --
        # this repo only ever used a single optimizer anyway).
